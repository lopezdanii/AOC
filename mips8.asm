...
InvierteCadena:
	addi $sp, $sp, -4  	#Apilo el caracter en la pila
	sw $s0, 0($sp)
	li $t1, 0   	#Contador de la longitud
bucle:	add $t6,$t1, $a1	
 	lb $s0, 0($t6) 		#Leo un caracter de la cadena origen
	beqz $s0, desapila 	#Si es el fin de cadena, he acabado de leer
...
desapila:
 ...
 bucle2:
 	...
 	add $t3,$t2, $a1
 	sb $s0, 0($t3) 		#La guardo en la cadena de destino
 	addi $t2, $t2, 1
 	...
 	add $v0,$a1,$zero
 	lw $s0, 0($sp) 		#Desapilo una letra de la pila
 	addi $sp, $sp, 4
